...
class Forms:
    class RegisterForm(FlaskForm):
        ...
        surname = StringField(label="Vorname",render_kw={"placeholder": "Vorname"}, default="", validators=[Length(min=0, max=30, message="Dein Vorname darf maximal 20 Zeichen lang sein.")])
        lastname = StringField(label="Nachname",render_kw={"placeholder": "Nachname"}, default="",validators=[Length(min=0, max=30, message="Dein Nachname darf maximal 20 Zeichen lang sein.")])
        ...
        street = StringField(render_kw={"placeholder": "Straße"},default="", validators=[
                                                                                            Length(min=0, max=25, message="Der Straßenname darf maximal 25 Zeichen lang sein.")])
        street_no = StringField(render_kw={"placeholder": "Nr"},default="",validators=[
                                                                                       Length(min=0, max=25, message="Die Straßennummer darf maximal 25 Zeichen lang sein.")])
        city = StringField( render_kw={"placeholder": "Stadt"},default="", validators=[
    ...
    class ContactForm(FlaskForm):
        category = SelectField(render_kw={"placeholder": "Art der Anfrage"},choices=[('kontakt', 'Kontakt'), ('mieten', 'Das Plan B Mieten'), ('feedback', 'Kritik/Lob'), ('events', 'Vorschlag für ein Event')], validators=[Length(min=1, max=20)])
        surname = StringField(render_kw={"placeholder": "Vorname"}, validators=[Length(min=0, max=20, message="Dein Vorname darf maximal 20 Zeichen lang sein.")])
        lastname = StringField(render_kw={"placeholder": "Nachname"}, validators=[Length(min=0, max=20, message="Dein Nachname darf maximal 20 Zeichen lang sein.")])
        email = StringField(render_kw={"placeholder": "Email"}, validators=[
                                                                            ...
                                                                            Email(message="Deine Email passt nicht ins Muster.")])
